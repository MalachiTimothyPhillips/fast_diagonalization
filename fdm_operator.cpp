    ...
    device(device_)
{
    ...
    if(N <= 12){
      fdm_operator_kernel = device.buildKernel("fdm_operator.okl", "two_d_fdm_operator", properties);
    } else {
      fdm_operator_kernel = device.buildKernel("fdm_operator.okl", "high_order_two_d_fdm_operator", properties);
    }
}
